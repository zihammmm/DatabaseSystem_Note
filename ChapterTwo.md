...
+ 二维表
> 由表框架与元组所组成，表框架有若干个属性组成  
> 一行数据被称为一个**元组**或**行**  
> 一张二维表由一个有n个属性的框架及m个元组组成
...
+ 关系中的基本概念
  ...
  + 元组
    > 关系中的每一行
  + 关键字
    > 关系中的一个属性集的值能唯一标识关系中的一个元组，且不含**多余的属性值**  
    > 每一个关系都有关键字  
    > 可以有多个关键字(候选关键字)    
    + 主关键字
        > 从候选关键字中选取一个
...
## 计算机世界与物理模型
主要涉及操作系统级文件组织，有时涉及到硬件级数据组织  

文件系统的组成
+ 项：最小基本单位，项内符号不能继续分割
+ 记录：各项间有内在语义联系(有型与值的区别)
+ 文件：记录的集合(一个文件所包含的记录是同型的)
+ 文件集：由若干个文件构成
